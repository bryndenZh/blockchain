    ...
    async def send_request(self, message, i=-1):
        ...
        while 1:
            try:
                ...
                self._log.info("client %d's  message %d processed successfully.", self._client_id, i)
                is_sent = True
            if is_sent:
                break
...
def setup(args = None):
    logging_config(log_level=logging.DEBUG, log_file="~$client_" + str(args.client_id) + ".log")
